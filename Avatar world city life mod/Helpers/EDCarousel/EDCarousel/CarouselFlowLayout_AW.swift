...
public enum CarouselFlowLayoutSpacingMode_AW {
...
public class CarouselFlowLayout_AW: UICollectionViewFlowLayout {
   ...
   public struct LayoutState_AW {
        var size: CGSize
        var direction: UICollectionView.ScrollDirection
        func isEqual_AW(_ otherState: LayoutState_AW) -> Bool {
    ...
    public var spacingMode = CarouselFlowLayoutSpacingMode_AW.fixed(spacing: 10)
    /// The current state of the layout, including the size and scroll direction of the collection view.
    public var state = LayoutState_AW(size: CGSize.zero, direction: .horizontal)
    ...
    /// If the state has changed, it calls the setupCollection_AWView() and updateLayout() methods.
    public override func prepare() {
        super.prepare()
        let currentState = LayoutState_AW(size: collectionView!.bounds.size, direction: scrollDirection)

        if !state.isEqual_AW(currentState) {
            setupCollection_AWView()
            updateLayout_AW()
            state = currentState
        }
    }
    
    /// The setupCollection_AWView() method is responsible for configuring the collection view.
    /// It sets the deceleration rate of the collection view to .fast if it is not already set to that value.
    public func setupCollection_AWView() {
    ...
    public override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        return attributes.map { self.transformLayoutAttributes_AW($0) }
    }
    ...
    public func transformLayoutAttributes_AW(_ attributes: UICollectionViewLayoutAttributes) -> UICollectionViewLayoutAttributes {
