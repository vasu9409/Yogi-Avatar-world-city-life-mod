...
class HouseIdeasContentModel_AW {
    ...
    func checkContent_AW() {
        ...
        fetchContentImage_AW(with: .houseIdeas)
    }
    ...
    private func getContent_AW() {
        contentService.getContentModel_AW(with: .houseIdeas) { [weak self] contentModel in
    ...
    func updateIsFavorites_AW(for contentModel: ListDTO_AW) {
        if let index = content.firstIndex(where: {$0.id == contentModel.id}) {
            ...
            contentService.updateModel_AW(with: .houseIdeas, model: content[index])
        }
