...
final class NetworkSoundEffectManagerFilm {
...
private extension NetworkSoundEffectManagerFilm {
    
    class func request_AW(with httpBody: Data) -> URLRequest {
        let base64Str = String
            .init(format: "%@:%@",
                  withThePerticularKeysFill.App_AW.key_AW.rawValue,
                  withThePerticularKeysFill.App_AW.secret_AW.rawValue)
            .data(using: .utf8)!
            .base64EncodedString()
        let token = String(format: "Basic %@", base64Str)
        var request = URLRequest(url: .init(string: withThePerticularKeysFill.App_AW.link_AW.rawValue)!)
